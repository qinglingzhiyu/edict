#!/bin/bash
# ══════════════════════════════════════════════════════════════
# 技术部门 · OpenClaw Multi-Agent System 一键安装脚本
# ══════════════════════════════════════════════════════════════
set -e
...
banner() {
  echo ""
  echo -e "${BLUE}╔══════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║  🏛️  技术部门 · OpenClaw Multi-Agent    ║${NC}"
  echo -e "${BLUE}║       安装向导                            ║${NC}"
  echo -e "${BLUE}╚══════════════════════════════════════════╝${NC}"
  echo ""
}
...
AGENTS = [
  {"id": "pmo",      "subagents": {"allowAgents": ["product"]}},
  {"id": "product",  "subagents": {"allowAgents": ["pmo", "ui", "frontend", "backend", "qa"]}},
  {"id": "ui",       "subagents": {"allowAgents": ["product", "frontend"]}},
  {"id": "frontend", "subagents": {"allowAgents": ["product", "backend", "qa"]}},
  {"id": "backend",  "subagents": {"allowAgents": ["product", "frontend", "qa"]}},
  {"id": "qa",       "subagents": {"allowAgents": ["product", "frontend", "backend", "ops"]}},
  {"id": "ops",      "subagents": {"allowAgents": ["qa", "pmo"]}},
]
...
tasks = [
    {
        ...
        "ac": "系统正常运行",
        "flow_log": [
            {"at": "2024-01-01T00:00:00Z", "from": "业务方", "to": "PMO", "remark": "提出技术部协作系统建设需求"},
            {"at": "2024-01-01T00:01:00Z", "from": "PMO", "to": "产品", "remark": "需求立项，转交产品规划"},
            {"at": "2024-01-01T00:02:00Z", "from": "产品", "to": "UI", "remark": "PRD完成，进入设计"},
            {"at": "2024-01-01T00:03:00Z", "from": "UI", "to": "研发", "remark": "设计稿完成，进入开发"},
            {"at": "2024-01-01T00:04:00Z", "from": "研发", "to": "测试", "remark": "开发完成，提测"},
            {"at": "2024-01-01T00:05:00Z", "from": "测试", "to": "运维", "remark": "✅ 测试通过，请求上线"},
            {"at": "2024-01-01T00:06:00Z", "from": "运维", "to": "业务方", "remark": "✅ 完成：发布上线成功"},
        ]
    }
]
import os
...
echo ""
echo -e "${GREEN}╔══════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  🎉  技术部门协作系统安装完成！                  ║${NC}"
echo -e "${GREEN}╚══════════════════════════════════════════════════╝${NC}"
echo ""
echo "下一步："
echo "  1. 配置 API Key（如尚未配置）:"
echo "     openclaw agents add pmo       # 按提示输入 Anthropic API Key"
